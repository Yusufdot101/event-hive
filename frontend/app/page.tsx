...
import { getEvents } from "./utilities/event";
...
function App() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (
                userLoc &&
                (userLoc.lng !== defaultLocation.lng ||
                    userLoc.lat !== defaultLocation.lat)
            ) {
                node.flyTo({ center: userLoc, zoom: 15 });
            }
        })();
    }, [node]);
        ...
        <>
            ...
            {isSelectingLocation && (
                <div className="absolute bottom-[40px] right-[40px] rounded-[4px] overflow-hidden">
                    ...
                    <button
                        className="cursor-pointer hover:bg-muted hover:text-white text-nowrap w-full p-[12px] bg-red-500 text-white py-[4px]  duration-300"
